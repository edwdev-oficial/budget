...
#     global g_client, category_customized, g_df, g_collection
#     g_df = df
#     g_client = client
    ...
    if despesas_mensais:
        ...
        if doc_list:
            ...
            if valor * -1 > despesas_programadas:
                # st.write('O valor gasto é menor que a despesa programada')
                collection.update_one({"_id": _id}, {'$set': {'Valor Programado': despesas_programadas - valor * -1}})
                # data_temp = collection.find({"_id": _id})
                # data_temp_list = list(data_temp)
                # st.write(data_temp_list)
